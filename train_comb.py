...
model_S = prompt_UNet3D(2, 4, True, False, True).to(device)
#model_S = prompt_UNet3D_add(2, 4, True, False, False).to(device)
...
optimizer_g = optim.Adam(model_S.prompt_tf.parameters(), lr=lr_S, betas=(0.5, 0.999))
optimizer_d = optim.Adam(discriminator.parameters(),lr=lr_S, betas=(0.5, 0.999))
optimizer_c = optim.Adam(model_S.classifier.parameters(),lr=lr_C, betas=(0.97, 0.999))
scheduler_S = optim.lr_scheduler.StepLR(optimizer_S, step_size=step_size_S, gamma=0.1)
scheduler_C = optim.lr_scheduler.StepLR(optimizer_c, step_size=step_size_S, gamma=0.1) #step_size_S=5000
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epoch):
        ...
        for data6,data3,data12 in zip(enumerate(train6loader),enumerate(train3loader),enumerate(train12loader),):
            ...
            g_cp = d_cp[6:12, :]
            g_loss = 3*criterion_d(g_cp,g_t.unsqueeze(1))
            g_loss.backward()
            optimizer_g.step()
            #d
            ...
            writer.add_scalar('label/c_loss',c_loss,iter_num)
            ...
            writer.add_scalar('lr/c', optimizer_c.param_groups[0]['lr'], iter_num)
