...
def build_h5_dataset(data_path, target_path):
    ...
    for i in range(30,33):
        ...
        if (i==1 or i==10 or i==19 or i==21):
            continue
        #if (i==8):
        #   continue
        f_T1 = os.path.join(data_path, 'subject-%d-T1.nii.gz' %i)
#        img_T1, header_T1 = load(f_T1)
        img_T1 = sitk.GetArrayFromImage(sitk.ReadImage(f_T1))
        print(img_T1.shape)
        f_T2 = os.path.join(data_path, 'subject-%d-T2.nii.gz' %i)
        img_T2 = sitk.GetArrayFromImage(sitk.ReadImage(f_T2))
        #img_T2, header_T2 = load(f_T2)
#        f_l = os.path.join(data_path, subject_name + 'label.nii.gz')
#        labels, header_label = load(f_l)
#        labels = sitk.GetArrayFromImage((sitk.ReadImage(f_l)))
#        print(labels.shape)
...
#        labels_caffe = labels_caffe.transpose(0, 4, 3, 1, 2)
        with h5py.File(os.path.join(target_path, 'train_seg_nocut_%s.h5' % (i)), 'w') as f:
