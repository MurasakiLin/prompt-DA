...
#net = prompt_UNet3D_add(2,4,True,False,'mul').to(device)
#net = cls_UNet3D(2, 4, True, False, True).to(device)
net = prompt_UNet3D(2, 4, True, False, True).to(device)
#net = UNet3D(2,4,True,False).to(device)
def inference(i):
    ...
    label_flip_dims=[3,2]
    subject_id = i
    subject_name = 'iseg/subject-%d-' % subject_id

    f_T1 = os.path.join(test_path, subject_name + 'T1.nii.gz')
    f_T2 = os.path.join(test_path, subject_name + 'T2.nii.gz')
    f_l = os.path.join(test_path, subject_name + 'label.nii.gz')
    #f_T1 = os.path.join(test_path, 'T1.nii.gz')
    #f_l = os.path.join(test_path, 'label.nii.gz')
    ...
    spacing = label_img_itk.GetSpacing()
    origin = label_img_itk.GetOrigin()
    direction = label_img_itk.GetDirection()
    ...
    print('%s | %s | %2.2f | %2.2f | %2.2f | %2.2f |' % ( \
                ...
                dsc[2],
                np.mean(dsc)))
    whole_pred = sitk.GetImageFromArray(whole_pred)
    whole_pred.SetSpacing(spacing)
    whole_pred.SetOrigin(origin)
    whole_pred.SetDirection(direction)
    path = os.path.join(outdir,  'subject-%d-label.nii.gz'%subject_id)
    sitk.WriteImage(whole_pred,path)
